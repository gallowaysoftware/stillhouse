...
Stage 208 shipped the report and stage 218 sourced most of it: Ontario,
Alberta and British Columbia now carry the programmes' own published
rates, banded by container size, and Québec correctly refuses. A report
covering only those three is remittable.

What is left is two provinces and one obligation nobody publishes per
container:

- **PEI and Nova Scotia** are still `Indicative`, so a report touching
  either is a planning figure. Each is one page on the province's own
  site. Nova Scotia needs care: it runs a **half-back** system, so the
  deposit collected and the deposit refunded are different numbers and a
  single rate cannot express both. PEI charges a non-refundable
  environment fee beside the deposit that the current figure excludes.
- **Stewardship fees outside BC.** Ontario (Circular Materials), Québec
  (Éco Entreprises Québec) and Alberta (ABCRC) all bill the producer, but
  on reported supply or tonnage rather than at a published per-container
  rate — so unlike BC's CRF these cannot be looked up, only read off the
  distillery's own account with each body. The report already says so per
  line rather than reporting a fee of zero.
