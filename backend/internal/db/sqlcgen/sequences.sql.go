...
const lockDocumentSequence = `-- name: LockDocumentSequence :exec

SELECT pg_advisory_xact_lock(
    hashtextextended(
        COALESCE(current_setting('app.current_tenant_id', true), '') || ':' || $1::text,
        0))
`
...
func (q *Queries) LockDocumentSequence(ctx context.Context, counter string) error {
	_, err := q.db.Exec(ctx, lockDocumentSequence, counter)
	return err
}
